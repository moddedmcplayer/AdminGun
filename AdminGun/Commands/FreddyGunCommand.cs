namespace AdminGun.Commands
{
    ...
    public class FreddyGunCommand : ICommand
    {
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
            if (!Plugin.Instance.Config.EnableVisualGun)
            {
                response = "Freddy is disabled.";
                return false;
            }
            if(sender is PlayerCommandSender plySender)
            {
                if (!plySender.CheckPermission("freddygun"))
                {
                    response = "Missing permission: freddygun";
                    return false;
                }
                
                if(arguments.Count == 0)
                {
                    response = "Usage: freddygun <player> | freddygun remove <player>";
                    return false;
                }
                ...
                if (arguments.At(0) != "remove")
                {
                    ...
                    var item = ply.AddItem(ItemType.GunCOM15);
                    if (item is Firearm firearm)
                    {
                        var flags = FirearmStatusFlags.MagazineInserted | FirearmStatusFlags.Chambered |
                                    FirearmStatusFlags.Cocked;
                        firearm.ApplyAttachmentsCode(77, true);
                        firearm.Status = new FirearmStatus(69, flags, firearm.GetCurrentAttachmentsCode());
                    }

                    ply.ReceiveHint(
                        ply.CheckPermission("freddygun")
                            ? Plugin.Instance.Config.AuthorizedObtainMessageFreddy
                            : Plugin.Instance.Config.UnauthorizedObtainMessageFreddy, 6);
                    Plugin.FreddySerials.Add(item.ItemSerial);
                ...
                else
                {
                    ...
                    {
                        response = "Usage: freddygun remove <player>";
                        return false;
                    }
                    ...
                    if (arguments.At(1).ToLower() == "unauthorized")
                    {
                        ...
                        {
                            if(Player.Get(hub).CheckPermission("freddygun"))
                                continue;
                            foreach (var kvp in hub.inventory.UserInventory.Items)
                            {
                                if(Plugin.FreddySerials.Contains(kvp.Key))
                                {
                    ...
                    else if (arguments.At(1).ToLower() == "all")
                    {
                        ...
                        {
                            if(Plugin.FreddySerials.Contains(firearmPickup.Info.Serial))
                            {
                                removed++;
                    ...
                    if(!handled)
                    {
                        ...
                        {
                            if (Plugin.FreddySerials.Contains(item.ItemSerial))
                            {
                                toRemove.Add(item);
                            }
                        }
                    ...
                    {
                        Plugin.FreddySerials.Remove(ibase.ItemSerial);
        ...
        public string Command { get; } = "freddygun";
        public string[] Aliases { get; } = new string[] { "fg" };
        public string Description { get; } = "Funny jumpscare gun.";
    }
}